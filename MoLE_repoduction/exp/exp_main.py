...
class Exp_Main(Exp_Basic):
    ...
    def test(self, setting, test=0, flag='test', fixed_head=None, seperate_head=False):
        ...
        if True:
            # Save metrics separately to avoid shape issues
            np.save(folder_path + f'mae_{flag}.npy', mae)
            np.save(folder_path + f'mse_{flag}.npy', mse)
            np.save(folder_path + f'rmse_{flag}.npy', rmse)
            np.save(folder_path + f'mape_{flag}.npy', mape)
            np.save(folder_path + f'mspe_{flag}.npy', mspe)
            np.save(folder_path + f'rse_{flag}.npy', rse)
            np.save(folder_path + f'corr_{flag}.npy', corr)
            np.save(folder_path + f'pred_{flag}.npy', preds)
